...
MPU9255::MPU9255() // Uses I2C communication by default
{
  Wire.begin();
}

void MPU9255::getMres()
{
  switch (Mscale)
  {
...
void MPU9255::readAccelData(int16_t * destination)
{
  ...
  readBytes(MPU9255_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]);
...
void MPU9255::readGyroData(int16_t * destination)
{
  ...
  readBytes(MPU9255_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]);
...
int16_t MPU9255::readTempData()
{
  ...
  readBytes(MPU9255_ADDRESS, TEMP_OUT_H, 2, &rawData[0]);
...
void MPU9255::initMPU9255()
{
  ...
  writeByte(MPU9255_ADDRESS, PWR_MGMT_1, 0x00);
  ...
  // With the MPU9255, it is possible to get gyro sample rates of 32 kHz (!),
  // 8 kHz, or 1 kHz
  ...
  writeByte(MPU9255_ADDRESS, SMPLRT_DIV, 0x04);
  ...
  writeByte(MPU9255_ADDRESS, GYRO_CONFIG, c );
  ...
  writeByte(MPU9255_ADDRESS, ACCEL_CONFIG, c);
  ...
  writeByte(MPU9255_ADDRESS, ACCEL_CONFIG2, c);
  ...
  writeByte(MPU9255_ADDRESS, INT_ENABLE, 0x01);
  delay(100);
}
...
void MPU9255::calibrateMPU9255(float * gyroBias, float * accelBias)
{
  ...
  writeByte(MPU9255_ADDRESS, PWR_MGMT_1, READ_FLAG);
  delay(100);
  ...
  writeByte(MPU9255_ADDRESS, INT_ENABLE, 0x00);
  // Disable FIFO
  writeByte(MPU9255_ADDRESS, FIFO_EN, 0x00);
  // Turn on internal clock source
  writeByte(MPU9255_ADDRESS, PWR_MGMT_1, 0x00);
  // Disable I2C master
  writeByte(MPU9255_ADDRESS, I2C_MST_CTRL, 0x00);
  // Disable FIFO and I2C master modes
  writeByte(MPU9255_ADDRESS, USER_CTRL, 0x00);
  // Reset FIFO and DMP
  writeByte(MPU9255_ADDRESS, USER_CTRL, 0x0C);
  delay(15);

  // Configure MPU6050 gyro and accelerometer for bias calculation
  // Set low-pass filter to 188 Hz
  writeByte(MPU9255_ADDRESS, CONFIG, 0x01);
  // Set sample rate to 1 kHz
  writeByte(MPU9255_ADDRESS, SMPLRT_DIV, 0x00);
  ...
  // MPU-9150)
  writeByte(MPU9255_ADDRESS, FIFO_EN, 0x78);
  ...
  readBytes(MPU9255_ADDRESS, FIFO_COUNTH, 2, &data[0]);
  ...
  for (ii = 0; ii < packet_count; ii++)
  {
    ...
    readBytes(MPU9255_ADDRESS, FIFO_R_W, 12, &data[0]);
  ...
  readBytes(MPU9255_ADDRESS, XA_OFFSET_H, 2, &data[0]);
...
// deviation is a pass.
void MPU9255::MPU9255SelfTest(float * destination)
{
  ...
  writeByte(MPU9255_ADDRESS, CONFIG, 0x02);
  // Set full scale range for the gyro to 250 dps
  writeByte(MPU9255_ADDRESS, GYRO_CONFIG, 1<<FS);
  // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
  writeByte(MPU9255_ADDRESS, ACCEL_CONFIG2, 0x02);
  // Set full scale range for the accelerometer to 2 g
  writeByte(MPU9255_ADDRESS, ACCEL_CONFIG, 1<<FS);
  ...
  writeByte(MPU9255_ADDRESS, ACCEL_CONFIG, 0xE0);
  // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
  writeByte(MPU9255_ADDRESS, GYRO_CONFIG,  0xE0);
  ...
  selfTest[0] = readByte(MPU9255_ADDRESS, SELF_TEST_X_ACCEL);
  // Y-axis accel self-test results
  selfTest[1] = readByte(MPU9255_ADDRESS, SELF_TEST_Y_ACCEL);
  // Z-axis accel self-test results
  selfTest[2] = readByte(MPU9255_ADDRESS, SELF_TEST_Z_ACCEL);
  // X-axis gyro self-test results
  selfTest[3] = readByte(MPU9255_ADDRESS, SELF_TEST_X_GYRO);
  // Y-axis gyro self-test results
  selfTest[4] = readByte(MPU9255_ADDRESS, SELF_TEST_Y_GYRO);
  // Z-axis gyro self-test results
  selfTest[5] = readByte(MPU9255_ADDRESS, SELF_TEST_Z_GYRO);
...
void MPU9255::magCalMPU9255(float * bias_dest, float * scale_dest)
{
...
uint8_t MPU9255::writeByte(uint8_t deviceAddress, uint8_t registerAddress,uint8_t data)
{
...
uint8_t MPU9255::readByte(uint8_t deviceAddress, uint8_t registerAddress)
{
...
uint8_t MPU9255::readBytes(uint8_t deviceAddress, uint8_t registerAddress, uint8_t count, uint8_t * dest)
{
  // Initialize the Tx buffer
  Wire.beginTransmission(deviceAddress);
  // Put slave register address in Tx buffer
