...
bool ADP5350::setCharger(bool on){
  ...
  setCurrentLimit(3); //set to 300 mA current limit (should charge faster);
  return true;
  
}

bool ADP5350::setCurrentLimit(uint8_t rate){
  if (rate>15) return false;
  writeByte(ADP5350_ADDRESS,CHARGER_VBUS_ILIM, rate);

}

uint8_t ADP5350::chargerStatus(){
  uint8_t currentSituation = readByte(ADP5350_ADDRESS, CHARGER_STATUS1);
  uint8_t output = ((uint8_t)7)&currentSituation;
  if (output ==4) return 2;
...
uint8_t ADP5350::enableFuelGauge(bool on){
  ...
  if (on)writeByte(ADP5350_ADDRESS, FUEL_GAUGE_MODE, 0x01);
  else writeByte(ADP5350_ADDRESS, FUEL_GAUGE_MODE, 0x00);
...
bool ADP5350::voltageLDO3(uint8_t voltage){
  if (voltage>13 || voltage <0) return false;//out of spec
  uint8_t command = 15-voltage;
  voltage_LDO(3,command);
}
...
//DO NOT TOUCH FUNCTION BELOW UNLESS YOU KNOW WHAT YOU'RE DOING!!
